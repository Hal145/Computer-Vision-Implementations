...
def produceDescriptors(img, device, descType, num_descriptors, visualize=False):
...
def gridSearch(X, y):
    # Grid search for parameters kernel, gamma and c
    param_grid = {'C': np.logspace(-3, 2, 6), 'gamma': np.logspace(-3, 2, 6), 'kernel': [chi2_kernel, 'linear']}
    grid = GridSearchCV(svm.SVC(), param_grid, cv=5)  # 5-fold cross validation
    grid.fit(X, y)
    return grid.best_params_


def svcClassifier(features, labels, kernel, c, gamma):
    """if kernel == 'precomputed':
        kernel = chi2_kernel"""
    svmResult = svm.SVC(kernel=kernel, C=c, gamma=gamma)
    svmResult.fit(features, labels)
    return svmResult
...
def trainSVC(image_name, data_dir, no_clusters, feature_descriptor, num_descriptors,  device):
    ...
    params = gridSearch(scaled_quantizedFeatures, labels)
    print('Best parameters:', [params.get("kernel"), params.get("C"), params.get("gamma")])
    best_parameters = [[params.get("kernel"), params.get("C"), params.get("gamma")]]
    svc = svcClassifier(scaled_quantizedFeatures, labels, params.get("kernel"), params.get("C"), params.get("gamma"))

    return svc, cluster_ids_x, cluster_centers, scaled_quantizedFeatures, best_parameters, sc_X


def testSVC(data_dir, feature_descriptor, num_descriptors, cluster_ids_x, cluster_centers, nu_clusters,
            quantized_features, svc, sc_X, device):
    ...
    testResults = svc.predict(features)
    classes = svc.classes_
    return testResults, labels, classes, image_list
...
def run(dataset_name, data_dir, test_dir, feature_descriptor, num_descriptors, no_clusters, matrix_size):
    ...
    parameters = ['classAccuracies', 'mean_F1', 'meanBalancedAcc', 'meanImbalancedAcc', 'f1_scores_with_labels',
                  'f1_scores', 'best_classes', 'worst_classes', 'best_parameters']
    results = [
        {'classAccuracies': classAccuracies, 'mean_F1': mean_F1, 'meanBalancedAcc': meanBalancedAcc,
         'meanImbalancedAcc': meanImbalancedAcc,
         'best_classes': best_classes, 'worst_classes': worst_classes, 'best_parameters': best_parameters}]
